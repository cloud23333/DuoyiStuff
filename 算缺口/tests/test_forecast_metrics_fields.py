...
def test_hurdle_spike_does_not_inflate_forecast():
    ...
    spiked = flat + (40, 38)
    flat_m = compute_forecast_metrics(daily_sales=flat, stocking_days=7, is_hot_style=False)
    spiked_m = compute_forecast_metrics(daily_sales=spiked, stocking_days=7, is_hot_style=False)
