...
def test_one_off_spike_does_not_inflate_daily_mean():
    flat = compute_forecast_metrics(
        ...
        is_hot_style=False, stock_in_warehouse=10,
    )
...
def test_recent_collapse_drives_forecast_down_fast():
    metrics = compute_forecast_metrics(
        ...
        is_hot_style=False, stock_in_warehouse=20,
    )
...
def test_intermittent_uses_slow_mover_label():
    metrics = compute_forecast_metrics(
        ...
        is_hot_style=False, stock_in_warehouse=5,
    )
...
def test_hot_style_uses_higher_service_level():
    metrics = compute_forecast_metrics(
        daily_sales=(8, 9, 8, 9, 8, 9, 8), stocking_days=7,
        is_hot_style=True, stock_in_warehouse=20,
    )
    assert metrics.demand_profile == "稳定款"
    assert metrics.service_level == pytest.approx(SL_HOT)
    # quantile ≥ mean → period sales ≥ predictive mean
    assert metrics.forecast_stocking_period_sales >= metrics.predictive_mean


def test_stable_non_hot_uses_base_service_level():
    metrics = compute_forecast_metrics(
        daily_sales=(8, 9, 8, 9, 8, 9, 8), stocking_days=7,
        is_hot_style=False, stock_in_warehouse=20,
    )
...
def test_stockout_tail_not_treated_as_drop():
    metrics = compute_forecast_metrics(
        ...
        is_hot_style=False, stock_in_warehouse=0,
    )
