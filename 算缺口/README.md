...
- `发货保守度 α` 滑块（默认 `1.0`=正常发货，越小越保守、少发降积压）：点击 `预览发货量` 会在选好三个输入文件后生成「发货量与缺货风险随 α 变化」的曲线，并给出建议值（可点 `采用建议` 一键采用）
- 拖动滑块即可实时看到 `预计今日发货 ≈ N 件（比正常 ±Y%）`；不预览直接运行则按默认 α=1.0（结果与原行为一致）
